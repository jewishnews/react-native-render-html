...
export function _constructStyles({
  ...
  console.log('style', htmlAttribs.tagName, htmlAttribs.style);
...
function cssToRNStyle(css, styleset, { parentTag, emSize, ptSize, ignoredStyles, allowedStyles }) {
  ...
  const returnVal = Object.keys(css)
    .filter(key => (allowedStyles ? allowedStyles.indexOf(key) !== -1 : true))
    .filter(key => (ignoredStyles || []).indexOf(key) === -1)
    .map(key => [key, css[key]])
    .map(([key, value]) => {
      const transformed = transform([[key, value]], ['boxShadow', 'fontFamily']);

      let temp = [];
      Object.keys(transformed).map(function(key) {
        temp.push([key, transformed[key]]);
      });
    ...
    .map(([[key, value]]) => {
      ...
      if (typeof value === 'string') {
        ...
        if (value.search('%') !== -1) {
          if (PERC_SUPPORTED_STYLES.indexOf(key) !== -1) {
            return [key, value];
          } else {
            // unsupported percent values (e.g borderRadius)
            // idk how we can get the percentage in a device unit value
            const percentToNumber = parseFloat(value.replace('%', ''));
            // return [key, percentToNumber];
            return undefined;
          }
        }
        ...
        //      transform has done this already
        ...
        if (key === 'fontWeight') {
          // check for obscure font weights
          const val = value.toString();
          const acceptedFontWeights = [
            'normal',
            'bold',
            '100',
            '200',
          ...
          if (!acceptedFontWeights.includes(val)) {
            // round it up to the nearest hundred
            // if it's a number
            const num = Number(val);
            if (Number(val) !== NaN && Number(val) > 0 && Number(val) < 999) {
              return [key, `${Math.ceil(Number(val) / 100) * 100}`];
            }
            return undefined;
          }
        }
